...
// Shows top-level folders from RenderSourceRepository as quick-select buttons

interface ProjectFolderPickerProps {
  value: string;                      // currently selected project folder path
...
export function ProjectFolderPicker({ value, onSelect }: Readonly<ProjectFolderPickerProps>) {
  ...
  useEffect(() => {
    async function fetchProjects() {
      try {
        const res = await fetch(`/api/files/browse?path=${encodeURIComponent(ROOT_PATHS[1].path)}`);
        const data = await res.json();
        if (res.ok) {
          setFolders((data.items || []).filter((f: FileItem) => f.isDirectory));
        }
      } catch { /* ignore */ }
