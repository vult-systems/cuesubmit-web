...
class SimpleCueSubmit(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        # Target Machines (was Services - much clearer)
        render_layout.addWidget(QtWidgets.QLabel("Render On:"), 1, 0)
        self.services_combo = QtWidgets.QComboBox()
        self.services_combo.addItems(get_services())
        self.services_combo.setToolTip("Which machines/group should render this job.\nMust match tags set up in CueGUI Services.")
        # Try to set maya as default if available
        idx = self.services_combo.findText("maya")
        if idx >= 0:
            self.services_combo.setCurrentIndex(idx)
        render_layout.addWidget(self.services_combo, 1, 1, 1, 5)
        ...
        self.maya_file.setPlaceholderText("//10.40.14.25/RenderSourceRepository/...")
        ...
        self.output_folder.setPlaceholderText("//10.40.14.25/RenderOutputRepo/...")
    ...
    def browse_maya_file(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            ...
            "//10.40.14.25/RenderSourceRepository",
    ...
    def submit_job(self):
        ...
        try:
            ...
            # Build render command as string (like working version)
            maya_file = self.maya_file.text().replace("\\", "/")
            output = self.output_folder.text().replace("\\", "/") if self.output_folder.text() else ""
            camera = self.camera.text()
            ...
            # Build XML spec (matches working maya_submit_worker.py exactly)
