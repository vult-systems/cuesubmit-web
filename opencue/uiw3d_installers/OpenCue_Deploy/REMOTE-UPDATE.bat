...
::   REMOTE-UPDATE.bat \\<server>\<share>
:: ============================================================================
...
echo [UPDATE] Admin:  !ADMIN_FULL!
echo [UPDATE] Share:  !SHARE!
echo.

:: Delete any leftover task from a previous run
schtasks /delete /tn "OC_Update" /f >nul 2>&1
...
if errorlevel 1 (
    echo [UPDATE] ERROR: schtasks /create failed ^(exit %ERRORLEVEL%^)
    echo [UPDATE] Check admin credentials and task scheduler permissions.
    exit /b 1
)

:: Trigger it immediately (async - returns right away, task runs in background)
schtasks /run /tn "OC_Update"
if errorlevel 1 (
    echo [UPDATE] ERROR: schtasks /run failed ^(exit %ERRORLEVEL%^)
    exit /b 1
)

echo.
echo [UPDATE] Task launched. UPDATE.bat is running elevated as !ADMIN_FULL! in the background.
echo [UPDATE] Progress log: C:\OpenCue\logs\update.log
echo [UPDATE] RQD will restart ~2 minutes after update completes.
echo.
exit /b 0
